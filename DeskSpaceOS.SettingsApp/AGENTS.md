> **Top-level source of truth:** repo-wide guidance (build, architecture, project
> shape) lives in the root [`agent_instruction.md`](../agent_instruction.md). This
> file is the **narrower, authoritative reference for SettingsApp (WinUI 3) work
> only** and is referenced from the SPOT. Read the SPOT first, then this file when
> touching the SettingsApp project.

# Copilot Agent Instructions -- WinUI 3 / WinAppSDK

## Project Overview

This is a **WinUI 3** desktop application built on the **Windows App SDK** with MSIX tooling. The build **`Platform` is x64 only** (per `.slnx` and `<Platforms>x64</Platforms>`); the project publishes RuntimeIdentifiers `win-x86;win-x64;win-arm64`. Always read the `.csproj` for the authoritative values.
...
| Platforms | Read `<Platforms>` from `.csproj` (currently `x64`) |
| RuntimeIdentifiers | Read `<RuntimeIdentifiers>` from `.csproj` (currently `win-x86;win-x64;win-arm64`) |
...
Build outputs are centralized under the solution root `artifacts` folder through
...
# Example unpackaged debug output:
.\artifacts\bin\<ProjectName>\debug_win-$Rid\<ProjectName>.exe
```
...
- Quick smoke tests: `dotnet run -c Debug -p:Platform=$Platform` from the
  project folder.
- Launch a built executable directly from `.\artifacts\bin\<ProjectName>\...`
  only when you intentionally want to bypass `dotnet run`.
